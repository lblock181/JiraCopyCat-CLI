...
impl Database for JSONFileDatabase {
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let serialized_db = serde_json::to_string(&db_state)?;
        fs::write(&self.file_path, serialized_db)?;
        Ok(())
    }
}
...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        let mut  db_state: DBState = self.read_db()?;
        let next_id: u32 = db_state.last_item_id + 1;
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        let mut db_state: DBState = self.read_db()?;
        let next_int: u32 = db_state.last_item_id + 1;
        // verify epic exists before creating
        match db_state.epics.get_mut(&epic_id) {
            Some(target_epic) => {
                db_state.last_item_id = next_int;
                db_state.stories.insert(next_int, story);
                target_epic.stories.push(next_int);
            }
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in &db_state.epics.get(&epic_id).ok_or_else(|| anyhow!("Invalid id"))?.stories {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut db_state = self.read_db()?;
        let epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Invalid Epic id"))?;
        for (index, epic_story) in epic.stories.iter().enumerate() {
            if &story_id == epic_story {
                epic.stories.remove(index);
                break;
            }
        }
        db_state.stories.remove(&story_id).ok_or_else(|| anyhow!("Invalid story id"))?;
        self.database.write_db(&db_state)?;
        Ok(())
    }
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        let mut db_state = self.read_db()?;
        let epic: &mut Epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Invalid Epic id"))?;
        epic.status = status;
        self.database.write_db(&db_state)?;
        Ok(())
    }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db_state = self.read_db()?;
        let story: &mut Story = db_state.stories.get_mut(&story_id).ok_or_else(|| anyhow!("Invalid Story id"))?;
        story.status = status;
        self.database.write_db(&db_state)?;
        Ok(())
    }
